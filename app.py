...
def list_parasite_words(message):
    ...
    prev_btn = types.InlineKeyboardButton("Previous", callback_data="prev_page")
    next_btn = types.InlineKeyboardButton("Next", callback_data="next_page")
    markup.row(prev_btn, next_btn)

    parasite_words = requests.get(
        f"http://duryssoile.nu.edu.kz/api/v1.0/words?type=parasite&offset={cur_offset}&limit={word_limit}&sort=asc"
    )
    ...
    for word in json.loads(parasite_words.text):
        result += word["word"] + " -> " + word["correctVersions"][0]["word"] + "\n"

    bot.send_message(message.chat.id, result, reply_markup=markup)


# have to move it another func to reset variable
...
def callback_page_handler(callback):
    ...
    if callback.data == "prev_page":
        ...
        list_mispronounced_words(callback.message)
